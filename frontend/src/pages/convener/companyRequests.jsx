...
function CompanyRequests() {
  ...
  const handleVerifyCompany = async (companyId, companyName) => {
    ...
    try {
      ...
      if (data.success) {
        alert(
          `Company ${companyName} verified successfully! They can no longer login.`
        );
        setVerificationRequests((prev) =>
          prev.filter((req) => req.companyId !== companyId)
        );
      } else {
  ...
  const handleGrantAccess = async (companyId, companyName) => {
    if (
      !window.confirm(
        `Are you sure you want to grant access to ${companyName}? They will be able to login and modify marks again.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        alert(
          `Access granted to ${companyName} successfully! They can now login and modify marks.`
        );
        setAccessRequests((prev) => prev.filter((req) => req._id !== companyId));
      } else {
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg">Loading company requests...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Review and manage company verification and access requests.
          </p>
        </div>
            ...
            <div>
              ...
              {activeTab === "verification" ? (
                <p className="text-sm text-purple-700">
                  When you verify a company's submission request, their account
                ...
                <p className="text-sm text-green-700">
                  When you grant access to a company POC, their verification status
