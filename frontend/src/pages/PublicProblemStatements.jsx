      ...
      <div className="container mx-auto px-4 py-16 max-w-7xl">
        ...
        <div className="mb-8 flex justify-center">
          <input
            ...
            className="w-full max-w-md px-6 py-4 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-md transition-all text-lg"
          />
        </div>

        {/* Error Message */}
        {error && (
          <div className="mb-6 bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-xl max-w-3xl mx-auto">
        ...
        {!loading && filteredPS.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {filteredPS.map((ps) => (
              <div
                key={ps.id}
                className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200 flex flex-col transform hover:-translate-y-1"
              >
                {/* Card Header */}
          ...
          <div className="bg-white rounded-xl shadow-md p-8 border border-gray-200">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600 mt-2">Total PS</p>
              </div>
              <div className="text-center">
                <p className="text-4xl font-bold text-red-600">
                  {problemStatements.filter((ps) => ps.prep === "high").length}
                </p>
                <p className="text-sm text-gray-600 mt-2">High Prep</p>
              </div>
              <div className="text-center">
                <p className="text-4xl font-bold text-yellow-600">
                  {problemStatements.filter((ps) => ps.prep === "mid").length}
                </p>
                <p className="text-sm text-gray-600 mt-2">Mid Prep</p>
              </div>
              <div className="text-center">
                <p className="text-4xl font-bold text-green-600">
                  {problemStatements.filter((ps) => ps.prep === "low").length}
                </p>
                <p className="text-sm text-gray-600 mt-2">Low Prep</p>
              </div>
            </div>
          </div>
        )}
