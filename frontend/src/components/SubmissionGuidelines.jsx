...
export default function SubmissionGuidelines({ variant = "full" }) {
  const steps = [
    ...
    },
    {
      ...
      description: "Click 'Submit' only when all deliverables are ready. Your submission time will be captured immediately."
    }
  ];

  const tips = [
    "Submission time is recorded when you click 'Submit', not when you upload individual files.",
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6 sticky top-6 h-fit">
      ...
      <div className="border-t border-gray-200 pt-6">
        ...
        <div className="space-y-3">
          {tips.map((tip, idx) => (
            <div key={idx} className="flex gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
      ...
      <div className="mt-6 p-4 bg-gray-50 border border-gray-200 rounded-md">
        ...
        <p className="text-xs text-gray-600">
          If you face any issues during submission, contact your convenor or refer to the help documentation.
        </p>
      </div>
    </div>
  );
